...
public class CalificacionWaisDAO {
	...
	public int[] obtenerPuntuacionCompuesta(String idIndice) {
		...
		return mongoOperations.find(puntuacionCompuesta, PuntuacionCompuestaWAIS.class,
				COLLECTION_CONVERSION_PUNTUACION_COMPUESTA).get(0).getPuntuacionCompuesta();
	}
	
	public double[] obtenerPercentil(String idIndice) {
		Query percentil = query(where("idIndice").is(idIndice));
		return mongoOperations.find(percentil, PuntuacionCompuestaWAIS.class,
				COLLECTION_CONVERSION_PUNTUACION_COMPUESTA).get(0).getPercentil();
	}
	
	public String[] obtenerIntervaloDeConfianza(String idIndice) {
		Query intervaloConfianza = query(where("idIndice").is(idIndice));
		return mongoOperations.find(intervaloConfianza, PuntuacionCompuestaWAIS.class,
				COLLECTION_CONVERSION_PUNTUACION_COMPUESTA).get(0).getIntervaloConfianza();
	}
